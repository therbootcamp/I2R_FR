...
# Affichage des statistiques pour Nachte et Region
tour %>%
  mutate(Nachte = Besucher * Dauer) %>%
  group_by(Region) %>%
  summarize(
...
```{r fig.height=5, fig.width=5.5, message=FALSE, warning=FALSE, dpi=300, eval=FALSE}
library(tidyverse) ; library(ggrepel)
...
```{r, message=F, warning=F, echo=T, eval=FALSE}
# Koeffizienten
summary(model)$coef[,-2]
```

```{r, message=F, warning=F, echo=F, eval=FALSE}
# Koeffizienten
summary(model)$coef[,-2]
```
]
...
```{r, message=F, warning=F, echo=F, fig.width=5,fig.height=5,dpi=300, eval = FALSE}
library(tidyverse) ; library(rpart)
library(rattle)
